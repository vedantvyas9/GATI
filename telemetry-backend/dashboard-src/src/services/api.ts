...
class APIClient {
  ...
  async fetchAnalyticsTimeseries(
    days?: number
  ): Promise<import('../types').AnalyticsTimeseriesResponse> {
    const response = await this.client.get('/analytics/timeseries', {
      params: typeof days === 'number' ? { days } : undefined,
    })
    return response.data
  }

  async fetchEngagementAnalytics(): Promise<import('../types').EngagementAnalyticsResponse> {
  ...
  async fetchUserAnalytics(
    ...
    search?: string,
    sortBy?: string
  ): Promise<import('../types').UserAnalyticsResponse> {
  ...
  async fetchFeatureAdoption(): Promise<import('../types').FeatureAdoptionResponse> {
    const response = await this.client.get('/analytics/features')
    return response.data
  }

  async fetchRetentionGrowth(): Promise<import('../types').RetentionGrowthResponse> {
    const response = await this.client.get('/analytics/retention')
    return response.data
  }

  async fetchInfrastructureInsights(
    days: number = 30
  ): Promise<import('../types').InfrastructureInsightsResponse> {
    const response = await this.client.get('/analytics/infrastructure', {
      params: { days },
    })
    return response.data
  }
